...
import { FaLink, FaEdit, FaKey, FaClipboardList, FaSpinner } from 'react-icons/fa';
...
const MethodDetails = () => {
  ...
  const performApiTest = async () => {
    ...
    // Check if methodDetails is loaded
    if (!methodDetails) {
      console.error("Method details are not loaded yet.");
      setLoadingApi(false);
      return;
    }

    const { mainUrl, method, headers = [] } = methodDetails;
    const accountUrl = selectedAccount.url; // Assuming selectedAccount is the account data

    // Construct the full URL
    const fullUrl = `${accountUrl}${mainUrl.startsWith('/') ? '' : '/'}${mainUrl}`;
  ...
  const handleTest = () => {
    const { queryParams } = methodDetails || {};

    // Check for created_at_min and created_at_max in queryParams
    ...
    // If either parameter exists, open the date modal
    if (hasCreatedAtMin || hasCreatedAtMax) {
      setIsDateModalOpen(true);
    } else {
      // Proceed with the API test if no date selection is needed
      performApiTest();
    }
  };
  ...
  const handleDateSubmit = () => {
    ...
    if (createdAtMin && createdAtMax) {
      performApiTest(); // Call the API test with the selected dates
    ...
    <div className="p-6 bg-gray-900 min-h-screen text-white">
     ...
     {selectedAccount && (
       <div className="mt-6 bg-gray-800 p-4 rounded-lg shadow-md flex flex-col gap-2">
         ...
         {/* Test API Button */}
         <button onClick={handleTest} className="mt-4 bg-green-600 text-white w-[200px] font-bold py-2 px-4 rounded hover:bg-green-700 transition duration=200" disabled={loadingApi}>
     ...
     {isDateModalOpen && (
       <Modal onClose={() => setIsDateModalOpen(false)}>
         ...
         <button onClick={handleDateSubmit} className="bg-blue=600 text-white py-2 px-4 rounded bg-blue-600 mt-4 hover:bg-blue-700 transition duration-200">Submit</button>
       </Modal>
     )}
   </div>
 );
